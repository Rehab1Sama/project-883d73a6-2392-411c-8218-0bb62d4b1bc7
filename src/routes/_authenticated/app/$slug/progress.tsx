...
function ProgressPage() {
  const { tenant, canRead, canRecordForCircle, canViewCircleById, loading, hasFeature, featuresLoading } = useTenantContext();
  ...
  // تُصفَّى الحلقات وفق نطاق المستخدمة: القيادة/النائبة الأكاديمية غير
  // المقيَّدة ترى كل الحلقات، والمعلمة/المشرفة المقيَّدة ترى حلقتها/حلقاتها
  // المسندة فقط عبر canViewCircleById (تُبنى من myCircleScopes).
  const visibleCircles = allCircles.filter((c) => canViewCircleById(c.id));
  ...
  const quotasQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("quotas")
        .select("student_id, track_id, category, target_amount, from_surah, from_ayah, to_surah, to_ayah")
        .eq("tenant_id", tenant!.id);
      if (error) throw error;
      return data;
    },
  ...
  useEffect(() => {
    ...
    for (const q of quotaRows ?? []) {
      if (q.track_id === trackId) t[`${q.student_id}|${q.category}`] = rangeFrom(q);
    }
  ...
  const saveQuotas = useMutation({
    mutationFn: async () => {
      ...
      const rows = circleStudents
        .flatMap((s) => cats.map((cat) => ({ s, cat, v: targetRanges[keyOf(s.id, cat)] ?? emptyRange })))
      ...
      const { error } = await supabase
        .from("quotas")
        .upsert(rows, { onConflict: "student_id,track_id,category" });
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const saveProgress = useMutation({
    mutationFn: async () => {
      ...
      // لو طالبة صارت "غايبة" وعندها نصاب محفوظ سابقًا لنفس اليوم، لازم
      // يُحذف — ما يصح تكون غايبة وعندها نصاب مسجَّل بنفس الوقت.
      if (absentIds.size) {
      ...
      if (rows.length) {
        const { error } = await supabase.from("progress_records").upsert(rows, {
          onConflict: "student_id,track_id,category,record_date",
        });
        if (error) throw error;
      }
          ...
          <>
            <div className="space-y-4">
              {circleStudents.map((s) => {
                ...
                const targetTotal = cats.reduce(
                  (sum, c) => sum + (rangePages(targetRanges[keyOf(s.id, c)] ?? emptyRange) ?? 0),
                  0,
                );
                return (
                  <section
                  ...
                  >
                    ...
                    {cats.map((cat) => {
                      ...
                      const t = targetRanges[tKey] ?? emptyRange;
                      ...
                      return (
                        <div key={cat} className="space-y-3 rounded-xl border border-border/70 p-4">
                          ...
                          <AyahRangePicker
                            ...
                            onChange={(v) => setTargetRanges({ ...targetRanges, [tKey]: v })}
                          />
                          <AyahRangePicker
                            label="منجز اليوم"
                            value={d}
